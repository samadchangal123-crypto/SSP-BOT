...
const USER_GROUP_DATA = path.join(__dirname, '../rdx-data/userGroupData.json');
...
async function handleChatbotResponse(sock, chatId, message, userMessage, senderId) {
    ...
    try {
        ...
        let startsWithBot = false;

        // Check if message starts with the word "bot" (case-insensitive)
        // Only triggers when "bot" is at the very start, not in middle or end
        if (userMessage && /^\s*bot\b/i.test(userMessage)) {
            startsWithBot = true;
        }
        ...
        // Only trigger when message STARTS with the word "bot".
        // Ignore mentions/replies so that "bot" appearing anywhere else (middle/end)
        // or replying to a previous bot message will NOT trigger a response.
        if (!startsWithBot) return;
        ...
        if (startsWithBot) {
            // Strip the leading "bot" trigger word from the message before sending to AI
            cleanedMessage = cleanedMessage.replace(/^\s*bot\b[\s,:!.-]*/i, '').trim();
        }

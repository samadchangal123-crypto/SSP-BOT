...
async function ensureDefaultSticker(state) {
        // No-op: default sticker support has been removed; fallback is a random RDX image
        return;
}
...
async function handleMentionDetection(sock, chatId, message) {
        try {
                ...
                // Helper: send a random RDX image as default reply
                const sendDefaultRdxImage = async () => {
                        const rdxImages = ['RDX1.jpg', 'RDX2.jpg', 'RDX3.jpg', 'RDX4.jpg', 'RDX5.jpg', 'RDX6.jpg'];
                        const pick = rdxImages[Math.floor(Math.random() * rdxImages.length)];
                        const p = path.join(__dirname, '..', 'assets', pick);
                        if (fs.existsSync(p)) {
                                await sock.sendMessage(chatId, { image: fs.readFileSync(p), caption: 'Hi 👋' }, { quoted: message });
                        } else {
                ...
                if (!state.assetPath) {
                        await sendDefaultRdxImage();
                        return;
                }
                const assetPath = path.join(__dirname, '..', state.assetPath);
        // If configured asset does not exist, send a random RDX image as a safe default

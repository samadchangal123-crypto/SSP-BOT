...
const { UploadFileUgu, UploadFileCatbox, TelegraPh } = require('../lib/uploader');
    ...
    try {
        ...
        try {
            if (media.ext === '.jpg' || media.ext === '.png' || media.ext === '.webp') {
                // Telegraph for images (permanent)
                try {
                    url = await TelegraPh(tempPath);
                } catch {
                    // Fallback: catbox.moe (permanent, no expiry)
                    url = await UploadFileCatbox(tempPath);
                }
            } else {
                // Catbox.moe for video/audio/docs — permanent, no expiry
                try {
                    url = await UploadFileCatbox(tempPath);
                } catch {
                    // Last fallback: uguu.se (48h expiry)
        ...
        const isPermLink = url.includes('catbox.moe') || url.includes('telegra.ph');
        const linkNote = isPermLink
            ? '✅ *Permanent Link* — Kabhi expire nahi hogi!'
            : '⚠️ *Temporary Link* — 48 ghante mein expire ho jayegi.';

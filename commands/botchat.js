const responses = [
    ...
    "RDX BOT Ko Bol Dunga Me Mujhe Paresan Kiya To",
    ...
    "chomu Tujhe Aur Koi Kaam Nhi H? Har Waqt Messenger pe Bot Bot Karta H",
    ...
    "Bar Bar Disturb Na Karen RDX BOT JaNu Ke SaTh Busy Hun 🤭🐒",
    ...
    "Hayee Mar Jawa Babu Ek Chuma To Doo Kafi Din Sy Chumi Nahi Mili Kahan Thy Babu inbox Ah Jao 😚🙈♥️",
    "Dur Dur karib na a  tjhe Aur Koi Kam Nahi Kiya Har Waqat Mjhy Tang Karte Rahte Ho 😂",
...
async function handleBotChat(sock, chatId, userMessage, senderId) {
    if (!userMessage) return;

    // Trigger ONLY when the message is exactly the word "bot" by itself
    // (case-insensitive, ignoring surrounding whitespace and trailing punctuation).
    // Anything else like "Hi bot", "yeh aik bot hai", etc. will be ignored.
    const isJustBot = /^\s*bot\s*[!.?,]*\s*$/i.test(userMessage);
    if (!isJustBot) return;

    const now = Date.now();
    if (now - lastBotResponseTime < COOLDOWN) return;
...
module.exports = { handleBotChat, responses };